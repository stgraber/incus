...
func (d *zfs) prefillCachedProperties(dataset string) {
	...
	if ok {
		...
		for _, propName := range zfsCacheProperties {
			...
			if !ok || prop.Expiry.Before(time.Now()) {
				valid = false
				break
			}
		}
	...
	if err != nil {
		d.logger.Warn("Coulnd't cache ZFS properties", logger.Ctx{"err": err})
	...
	for _, line := range strings.Split(out, "\n") {
		...
		for i, value := range fields[1:] {
			...
			record[key] = zfsCacheEntry{Expiry: time.Now().Add(15 * time.Second), Value: value}
		}
